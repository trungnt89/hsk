    ...
    async toggleVoiceList(sync = false) {
        ...
        files.sort((a,b) => (b.date || 0) - (a.date || 0)).forEach(async f => {
            ...
            item.innerHTML = `
                ...
                <div style="font-size:12px; display:flex; justify-content:space-between"><span>🕒 ${f.formattedDate || (f.date ? new Date(f.date).toISOString().replace('T', ' ').split('.')[0] : 'N/A')}</span><b>${f.score || '0/1000'}</b></div>

(function() {
    ...
    function initAssistiveTouch() {
        console.log("[Log] Initializing 3x3 Compact Menu Fixed...");
        ...
        styleSheet.innerText = `
            ...
            .menu-container { position: relative; background: rgba(28, 28, 30, 0.95); padding: 22px 18px 18px 18px; border-radius: 30px; width: 240px; box-shadow: 0 20px 40px rgba(0,0,0,0.4); border: 0.5px solid rgba(255,255,255,0.1); font-family: -apple-system, sans-serif; }
            .menu-section-title { color: #8e8e93; font-size: 9px; font-weight: 700; text-transform: uppercase; margin: 8px 0 10px 4px; letter-spacing: 0.5px; border-bottom: 0.5px solid rgba(255,255,255,0.1); padding-bottom: 3px; width: 100%; box-sizing: border-box; }
            ...
            #close-menu { position: absolute; top: 12px; right: 12px; width: 22px; height: 22px; background: rgba(255,255,255,0.15); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: #efeff4; font-size: 10px; cursor: pointer; transition: all 0.2s; z-index: 20; font-weight: bold; }
        ...
        const onMove = (e) => {
            ...
            if (isDragging) {
                e.preventDefault();
                const x = Math.max(0, Math.min(cX - offset.x, window.innerWidth - 50));
                const y = Math.max(0, Math.min(cY - offset.y, window.innerHeight - 50));
        ...
        window.addEventListener('mouseup', () => { 
            if(isDragging) console.log("[Log] Drag ended at:", button.style.left, button.style.top);
